...
Written by hand rather than autogenerated, on purpose: it only adds the Phase A
tables and the one new column on games, leaving the pre-existing four alone.

Originally this had down_revision = None, because it was written against a
database where teams/games/users/play_by_play already existed (created by
create_all long before Alembic). That made it unrunnable on a fresh database —
it failed on its first statement, ALTER TABLE games. 0000_baseline now creates
those four, and this revision follows it, so `alembic upgrade head` builds the
whole schema from nothing.

Every operation here is additive.
"""
